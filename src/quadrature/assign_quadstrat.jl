struct AssignQuadStrat <: Operator
    op::Operator
    qs::AbstractQuadStrat
end
...
function assign_quadstrat(op::LinearCombinationOfOperators, qss::Vector{<:AbstractQuadStrat}, operator_types::Vector)
    return sum([c*assign_quadstrat(opi,qss[findfirst(typeof(opi),operator_types)]) for (c,opi) in zip(op.coeffs,op.ops)])
end
